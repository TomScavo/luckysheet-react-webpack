...
function mock(req, res) {
  ...
  console.log(
    '初次请求：method: ' + opt.method + '  host: ' + opt.host + '  port: ' + opt.port + '  path: ' + opt.path
  );
...
var mockUrl = appConfig.Mock_Target + appConfig.Mock_Base_Path;
var realUrl = appConfig.Real_Backend_Api_Host;
var port = appConfig.Mock_Server_Port ? appConfig.Mock_Server_Port : 36742;
var server = http.createServer(onProxy);
server.listen(port);
console.log('内网测试，Mock数据正监听端口：' + port);
